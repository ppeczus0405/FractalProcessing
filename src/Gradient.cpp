...
const Gradient Gradient::WHITE_BLACK({{0.0, RGB::WHITE},
                                      {0.5, RGB::GRAY},
                                      {1.0, RGB::BLACK}});
...
Gradient::Gradient(const std::vector<std::pair<double, RGB>>& gradient_points)
...
bool Gradient::insertPoint(const double x, const RGB& color) {
  ...
  while (ind >= 0 && CompareDoubles::isGreater(grad_points[ind].first,
                                               grad_points[ind + 1].first)) {
...
bool Gradient::changePoint(const double x, const RGB& color) {
  ...
  if (!removed)
    return false;  // Point wasn't inside
...
RGB Gradient::getPointColor(const double x) {
  // If point is inside then color will be returned else invalid argument
  // exception is thrown
  auto wanted = getPointIter(x);
  if (wanted == grad_points.end()) {
    throw std::invalid_argument(
        "The point at the position does not belong to the gradient set of "
        "points");
  }
  return wanted->second;
}

std::vector<std::pair<double, RGB>> Gradient::getPoints() noexcept {
  return grad_points;
}

std::vector<RGB> Gradient::generateGradientMap(int map_size) {
  ...
  if ((int)grad_points.size() == 0) {
    return std::vector<RGB>(map_size, RGB(0, 0, 0));
  }
  // One points inside. Gradient map is just color of the point
  if ((int)grad_points.size() == 1) {
    return std::vector<RGB>(map_size, grad_points.front().second);
  }
  ...
  if (map_size == 1) {
    return std::vector<RGB>(map_size, interpolate(begin + length / 2.0));
  }
...
bool Gradient::write(std::string filename, int w, int h) {
  if (w <= 0 || h <= 0)
    return false;
...
std::vector<std::pair<double, RGB>>::iterator Gradient::getPointIter(
...
void Gradient::generateMonotoneIntervals() {
  ...
  auto get_color_values = [this](std::function<uint8_t(RGB)> f) {
    std::vector<int> v;
  ...
  auto get_mono_intervals = [&](const auto& values) {
    std::vector<double> X{grad_points[0].first, grad_points[1].first};
    std::vector<double> Y{(double)values[0], (double)values[1]};
    std::vector<std::tuple<tk::spline, int, int>> intervals;
  ...
  mono_intervals[static_cast<uint8_t>(InterpolationTarget::RED)] =
      get_mono_intervals(rvalues);
  mono_intervals[static_cast<uint8_t>(InterpolationTarget::GREEN)] =
      get_mono_intervals(gvalues);
  mono_intervals[static_cast<uint8_t>(InterpolationTarget::BLUE)] =
      get_mono_intervals(bvalues);
  interpolated = true;
}

RGB Gradient::interpolate(const double x) {
  // If we have no points inside then interpolation value is BLACK for any x
  // value
  if ((int)grad_points.size() == 0) {
    return RGB(0, 0, 0);
  }
  ...
  auto get_interpolated = [this](const auto& v, double xx) -> double {
    auto is_inside = [](const double& a, const double& b, const double& val) {
      return (CompareDoubles::isGreater(val, a) ||
              CompareDoubles::isEqual(val, a)) &&
             (CompareDoubles::isLesser(val, b) ||
              CompareDoubles::isEqual(val, b));
    };
  ...
  // Anwser build
  int r = get_interpolated(
      mono_intervals[static_cast<uint8_t>(InterpolationTarget::RED)], x);
  int g = get_interpolated(
      mono_intervals[static_cast<uint8_t>(InterpolationTarget::GREEN)], x);
  int b = get_interpolated(
      mono_intervals[static_cast<uint8_t>(InterpolationTarget::BLUE)], x);
