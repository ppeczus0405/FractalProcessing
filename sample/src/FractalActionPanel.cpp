...
FractalActionPanel::FractalActionPanel(QWidget* parent) : QWidget(parent) {
  ...
  m_zoomOutButton->setIcon(
      QIcon(":/icons/zoomout.svg"));  // example resource path
  ...
  connect(m_generateButton, &QPushButton::clicked, this,
          &FractalActionPanel::generateFractal);

  connect(m_zoomOutButton, &QPushButton::clicked, this,
          &FractalActionPanel::zoomOutRequest);
}
